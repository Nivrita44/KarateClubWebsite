...
app.get("/api/transaction/:tran_id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Server error while fetching transaction details" });
  }
});
...
app.post("/sslcommerz/success/:tran_id", async (req, res) => {
  const { tran_id } = req.params;
  try {
    const [result] = await sslcommerzDb.execute(
      "UPDATE bills SET status = 'paid' WHERE transaction_id = ?",
      [tran_id]
    );
...
app.put("/api/students/:id", async (req, res) => {
  ...
  try {
    await db.query(
      "UPDATE students SET belt = ?, certificate = ? WHERE id = ?",
      [belt, certificate, studentId]
    );

    if (belt) {
      await db.query(
        "INSERT INTO notifications (studentId, message, type) VALUES (?, ?, ?)",
        [studentId, `Your belt has been updated to ${belt}.`, "belt"]
      );
    }
    if (certificate) {
      await db.query(
        "INSERT INTO notifications (studentId, message, type) VALUES (?, ?, ?)",
        [studentId, "Your certificate has been uploaded.", "certificate"]
      );
    }
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await db.query("UPDATE students SET imageUrl = ? WHERE id = ?", [
        imageUrl,
        userId,
      ]);
  ...
  try {
    for (const section in updates) {
      const [exists] = await db.query(
        "SELECT * FROM about_club WHERE section = ?",
        [section]
      );
      if (exists.length > 0) {
        await db.query("UPDATE about_club SET content = ? WHERE section = ?", [
          updates[section],
          section,
        ]);
      } else {
        await db.query(
          "INSERT INTO about_club (section, content) VALUES (?, ?)",
...
app.get("/api/exams/:belt", async (req, res) => {
  ...
  try {
    const [rows] = await db.query("SELECT * FROM exam_routine WHERE belt = ?", [
      belt,
    ]);
    res.json(rows);
  } catch (err) {
